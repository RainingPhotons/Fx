...
void particle_init(particle *p, size_t size) {
    ...
    init_vel = (((float) rand()) / ((float) RAND_MAX) * (-kLEDCNT) - (kLEDCNT / (rand() % 4 + 1))) * 0.8;
    ...
    //col = rand() % COLS;
    //changing this to specify a specific column
    col = 10;

    // initialize particles
    for (i = 0; i < size; ++i) {
        p[i].pos[0] = (int) (kLEDCNT - 1); // start particles at bottom
        p[i].pos[1] = col;
        p[i].vel[0] = init_vel;
        p[i].vel[1] = 0.0;
        p[i].life = 1.6;
